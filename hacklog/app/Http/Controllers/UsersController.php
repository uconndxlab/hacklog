...
class UsersController extends Controller
{
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($q) use ($search) {
                $q->where('name', 'like', '%'.$search.'%')
                    ->orWhere(fn ($nicknameQuery) => $nicknameQuery->whereNicknameLike($search))
     ...
     */
    private function nicknamesFromRequest(Request $request): ?array
    {
        $nicknames = User::parseNicknames($request->input('nicknames'));

        return $nicknames === [] ? null : $nicknames;
    }
